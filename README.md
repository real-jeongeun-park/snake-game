...
C언어로 구현한 콘솔 기반 Snake Game입니다. 고전 게임 Snake에서 아이디어를 얻어 제작했으며, 기존 Snake의 일부 기능은 제거하고 새로운 기능(Vitality, Time Checking 등)을 추가해 재구성했습니다.

## 📌 게임 소개

플레이어는 방향키로 Snake를 조작해 **최대한 오랫동안 생존**하는 것이 목표인 게임입니다.

- 시간이 지날수록 Snake의 생명력(**Vitality**)이 점차 감소합니다.
- 화면에 랜덤으로 생성되는 **Medicine(+)**을 먹으면 Vitality를 회복할 수 있습니다.
